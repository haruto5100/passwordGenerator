...
const ADVICE_TEMPLATES = {
    ...
    TOO_SHORT: (length) => `
        **改善点：長さの強化** - パスワードが${length}文字と短すぎます。現代のGPU攻撃から防御するためには、**最低12文字**を推奨します。理想的には16文字以上にしましょう。
    `,
    // 文字種が少ない場合
    LOW_COMPLEXITY: (missingTypes) => `
        **改善点：複雑さの追加** - ${missingTypes.join('と')}が含まれていません。文字種の多様性はエントロピーを劇的に増加させます。最低でも英大文字、小文字、数字、記号の**3種類以上**を組み合わせましょう。
    `,
    // 連続する文字や辞書語のようなパターンがある場合（エントロピーで間接的に警告）
    WEAK_PATTERN: `
        **改善点：パターン排除** - ユーザーが自作したパスワードの場合、誕生日やペットの名前など、推測されやすい**辞書語や連続パターン**の使用は避けてください。生成したパスワードであれば、そのまま利用しても問題ありません。
    `,
    // 非常に強力な場合
    VERY_STRONG: `
        **評価：非常に安全** - 素晴らしい！このパスワードは高いエントロピー（情報量）を持ち、ブルートフォース攻撃に対する耐性が非常に高いと評価されました。この強度を維持しましょう。
    `
};
